({
    ...
    addOliHelper : function(cmp,event,productId,type) {
        ...
        if(oppObj.Quote_Type__c == 'B2C - Renewal'){
            if(oppObj.Account.Subscription_End_Date__c<this.getToday()){
                ...
                /*let today = new Date(oppObj.Account.Subscription_End_Date__c);
    ...
    getpickListValues : function(cmp,event) {
        ...
        action.setCallback(this,result=>{
            cmp.set('v.pickList',result.getReturnValue());
        });
            $A.enqueueAction(action);
        },
            getToday: function() {
            ...
            validateFieldsForSG: function (cmp, listOppLine) {
                ...
                var listDiscountReasonNot100 = {'Loyalty Discount': true, 'Managerial Discount': true, 'Marketing Promo': true, 'First-Timer Discount': true, 'Agent Referral: Referee': true, 'Cash Voucher': true, 'Renewal Promotion' : true, 'Renewal Bonus' : true};
                ...
                if (Array.isArray(discountReason)) {
                    for (var oppLineIdx in listOppLine) {
                        ...
                        rowResult = this.proceedValidateStartEndDate(startDateField[oppLineIdx].get('v.value'), endDateField[oppLineIdx].get('v.value'), oppObj, listOppLine[oppLineIdx]);
                        if (!rowResult.result) {
                            result &= false;
                    ...
                    rowResult = this.proceedValidateStartEndDate(startDateField.get('v.value'), endDateField.get('v.value'), oppObj, listOppLine[0]);
                    if (!rowResult.result) {
                        result &= false;
                        if(rowResult.startDateMessage){
                            this.showToast(cmp, event, 'ERROR', rowResult.startDateMessage);
            ...
            /*proceedValidateStartEndDateForSG: function(startDate, endDate, oppObj, oppLineItem) {
                var result = true, startDateMessage = null, endDateMessage = null;
                ...
                if(strtDate != ostrtDate){
                    result &= false;
                    startDateMessage = 'Modification of Start Date Field of an Upgrade Opportunity is not allowed.';
                }
            }*/
            ...
            return result;
                           ...
                           /*proceedValidateStartEndDateForMY: function(
                           startDate,
                           endDate,
                           oppObj,
                           oppLineItem
        ...
        if (EndDate != oEndDate) {
          if (strtDate == ostrtDate) {
            ...
            proceedValidateStartEndDate : function(startDate, endDate, oppObj, oppLineItem) {
